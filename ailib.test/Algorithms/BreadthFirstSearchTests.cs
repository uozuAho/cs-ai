...
namespace ailib.test.Algorithms
{
    public class BreadthFirstSearchTests
    {
        ...
        public void Init()
        {
            var problem = A.Fake<ISearchProblem<StateMock, ActionMock<string>>>();
            var initialState = new StateMock("initial state");
            A.CallTo(() => problem.InitialState).Returns(initialState);
            A.CallTo(() => problem.GetActions(A<StateMock>._)).Returns(new List<ActionMock<string>>());
            A.CallTo(() => problem.IsGoal(A<StateMock>._)).Returns(false);
            
            var bfs = new BreadthFirstSearch<StateMock, ActionMock<string>>(problem);
