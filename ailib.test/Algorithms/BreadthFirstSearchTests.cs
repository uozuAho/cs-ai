...
namespace ailib.test.Algorithms
{
    public class BreadthFirstSearchTests
    {
        ...
        public void Init()
        {
            var problem = A.Fake<ISearchProblem<StateMock<string>, ActionMock<string>>>();
            var initialState = new StateMock<string> {Value = "initial state"};
            A.CallTo(() => problem.InitialState).Returns(initialState);
            A.CallTo(() => problem.GetActions(A<StateMock<string>>._)).Returns(new List<ActionMock<string>>());
            A.CallTo(() => problem.IsGoal(A<StateMock<string>>._)).Returns(false);
            
            var bfs = new BreadthFirstSearch<StateMock<string>, ActionMock<string>>(problem);
        ...
        {
            var action1 = new ActionMock<string> { Value = "action 1" };
            var action2 = new ActionMock<string> { Value = "action 2" };
            var initialState = new StateMock<string> { Value = "initial state" };
            var state1 = new StateMock<string> { Value = "state 1" };
            var state2 = new StateMock<string> { Value = "state 2" };
            var problem = A.Fake<ISearchProblem<StateMock<string>, ActionMock<string>>>();
            A.CallTo(() => problem.InitialState).Returns(initialState);
            A.CallTo(() => problem.GetActions(A<StateMock<string>>._)).Returns(new [] {action1, action2});
